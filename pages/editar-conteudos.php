...
<div class="box-content-conteudo">
    ...
    <h2><i class="bi bi-inboxes-fill"></i> Editar Condeúdo</h2>

	<!--<form class="ajax-form" id="formEditarConteudo" method="post" enctype="multipart/form-data"> sem este atributo ENCTYPE, nao funciona o upload da imagem-->
	<form id="formEditarConteudo" method="post" enctype="multipart/form-data"> <!--sem este atributo ENCTYPE, nao funciona o upload da imagem--> 
				...
				if($titulo == ''){
					...
					if($verifica->rowCount() == 0){
						...
						if($conteudo->alterarConteudos($id,$playlist_id,$titulo,$urldescricao,$author)){								
							Painel::alert('sucesso','Conteúdo foi alterada com sucesso!');  
							$conteudo = Conteudo::selecionarConteudo('tb_site_conteudos','id = ?',array($id));   							                     	
						}else{
							Painel::alert('erro','Erro ao alterar o conteudo!');
						}
					}else{
		...
		<div class="form-group">
            ...
            <select name="playlist_id">
                ...
                <?php
                    $playlists = Painel::selectAll('tb_site_playlists');
                    foreach ($playlists as $key => $value) {
                ?>
                <option <?php if($value['id'] == $conteudo['playlist_id']) echo 'selected'; ?> value="<?php echo $value['id'] ?>"><?php echo $value['description']; ?></option>
                <?php } ?>
            </select>
		</div>
		...
		<div class="form-group">
			<label>Título:</label>
			<input type="text" name="titulo" maxlength="150"  value="<?php echo $conteudo['title']; ?>" require>
		</div><!--form-group--> 

		<div class="form-group">
			<label>Url:</label>
			<input type="text" name="urldescricao" maxlength="255"  value="<?php echo $conteudo['urldescricao']; ?>" require>
		</div> 
		
		<div class="form-group">
			<label>Autor:</label>
			<input type="text" name="author" maxlength="150"  value="<?php echo $conteudo['author']; ?>" require>
		</div> 

		<div class="form-group">
			<input type="submit" name="acao" value="Atualizar">
			<input type="hidden" name="identificador" value="form_EditarConteudo">         <!--campos ocultos para servir de referencia-->
